...
from account.decorators import login_required, admin_required, super_admin_required
...
def _get_max_number(contest_id):
    rank = ContestRank.objects.filter(contest_id=contest_id). \
        select_related("user"). \
        order_by("-total_ac_number", "total_time"). \
        values("id", "user__id", "user__username", "user__real_name", "user__userprofile__student_id",
...
#push announcement api save data to database
